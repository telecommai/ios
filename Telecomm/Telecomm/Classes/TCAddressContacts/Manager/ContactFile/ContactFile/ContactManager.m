...
#import "ContactObjectManager.h"
#import "RITLContactObject+RITLContactFile.h"
#import "ContactCatcheManager.h"
...
- (void)__obtainContacts:(void (^)(NSArray<RITLContactObject *> * _Nonnull))completeBlock{
    ...
    //进行缓存
    [[ContactCatcheManager sharedInstace]startRequestIndentifiers:^(NSArray<NSString *> * _Nonnull identifiers) {
...
- (void)__contactDidChange:(NSNotification *)notication{
    ...
    if (self.contactDidChange != nil ){
        ...
        if (self.shouldResponseContactChange == true){
            //重新加载缓存
            [[ContactCatcheManager sharedInstace]reloadContactIdentifiers:^(NSArray<NSString *> * _Nonnull identifiers) {
...
- (NSArray <RITLContactObject *> *)__contactHandleWithIdentifiers:(NSArray <NSString *> *)identifiers{
    ...
    for (NSString * identifier in identifiers){
        ...
        //进行对象的处理
        RITLContactObject * contactObject = [ContactObjectManager contantObject:contact];
