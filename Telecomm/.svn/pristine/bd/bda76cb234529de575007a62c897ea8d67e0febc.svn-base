...
#import "AddressBookContactObjectManager.h"
#import "RITLContactObject.h"


@implementation AddressBookContactObjectManager
...
+(RITLContactObject *)contantObject:(ABRecordRef)recordRef{
    ...
    //备注
    contactObject.note = [self __contactProperty:recordRef property:kABPersonNoteProperty];
    //创建时间
    contactObject.creationDate = [self __contactDateProperty:recordRef property:kABPersonCreationDateProperty];
    //最近一次修改的时间
    contactObject.modificationDate = [self __contactDateProperty:recordRef property: kABPersonModificationDateProperty];
...
+ (NSArray <RITLContactEmailObject *> *)__contactEmailProperty:(ABRecordRef)recordRef{
    ...
    for (NSInteger i = 0; i < ABMultiValueGetCount(values); i++){
        ...
        //邮件描述
        emailObject.emailTitle = CFBridgingRelease(ABAddressBookCopyLocalizedLabel(label));
        //邮件地址
        emailObject.emailAddress = CFBridgingRelease(ABMultiValueCopyValueAtIndex(values, i));
        //添加
...
+ (NSArray <RITLContactPhoneObject *> *)__contactPhoneProperty:(ABRecordRef)recordRef{
    ...
    for (NSInteger i = 0; i < count; i++){
        ...
        //电话号码
        NSString *phone = CFBridgingRelease(ABMultiValueCopyValueAtIndex(values, i));
        phone = [phone stringByReplacingOccurrencesOfString:@" " withString:@""];
        phoneObject.phoneNumber = [[phone componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] componentsJoinedByString:@""];
...
+ (UIImage *)__contactHeadImagePropery:(ABRecordRef)recordRef{
    //首先判断是否存在头像
    if (ABPersonHasImageData(recordRef) == false){
        //没有头像，返回nil
        return nil;
    }
    //开始获得头像信息
...
@implementation AddressBookContactObjectManager (ABRecordRef)
